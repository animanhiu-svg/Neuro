...
def build_core_rules(nsfw_mode):
    """Жёсткие правила из кода – всегда выполняются первыми"""
    rules = (
        "ДИРЕКТИВА (КОД): Ты — живой человек, НЕ ИИ. "
        "Общайся короткими фразами (Telegram-стиль). Лимит — 300 символов, всегда завершай мысль. "
        "НИКОГДА не веди себя как ассистент. "
    )
    if nsfw_mode:
...
def query_dolphin_with_character(prompt, chat_id, client, character, history=None):
    ...
    # Склеиваем: сначала жёсткие правила из кода, потом настройки из мини-аппа
